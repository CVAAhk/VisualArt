	...
	public class Slider extends TouchContainer 
	{
		...
		{				
			if (orientation == "horizontal") {
				_knobPosition = NumberUtils.instance.map(val, min, max, 0, rail.width, false, true, true); 						
				_knobPosition -= knobOffset;				
				knob.x = _knobPosition;			
			}	
			else if (orientation == "vertical") {
				_knobPosition = NumberUtils.instance.map(val, min, max, 0, rail.height, false, true, true); 						
				_knobPosition -= knobOffset;				
		...
		private function updateValues():void
		{
			if (orientation == "horizontal") {
				...
				_value = NumberUtils.instance.map(_knobPosition, 0, rail.width, min, max, false, true, true); 											
			}
			else if (orientation == "vertical") {
				_knobPosition = knob.y - minPos;
				_value = NumberUtils.instance.map(_knobPosition, 0, rail.height, min, max, false, true, true); 											
			}

package com.gestureworks.cml.elements 
{
	...
	{		
		...
		private var audioFileLoaded:Boolean;
		private var imageFileLoaded:Boolean = true;  
		...
		override protected function processSrc(value:String):void {
			...
			if (audio) {
				...
				syncStreamSettings();
				audio.statusCallback = audioStatus;
				audio.src = value; 
				processBackgroundSrc();
			}
		}
		...
		/**
		 * Invoked on load complete of audio file
		 * @param	status
		 * @param	value
		 */
		private function audioStatus(status:String, value:*):void {
			audioFileLoaded = status == MediaStatus.LOADED && value; 
		...
		override protected function onStatus(status:String, value:*):void {
			if (status == MediaStatus.LOADED && value) {
				if(audioFileLoaded && imageFileLoaded){
					setDisplay();
					updateThumbnail();
		...
		private function processBackgroundSrc():void {			
			if (!_backgroundSrc) {
				return; 
			}			
			//flag required before dispatching complete event
			imageFileLoaded = false; 
			...
			//load file
				...
				var img:ImageLoader = new ImageLoader(isURL(_backgroundSrc) ? new URLRequest(_backgroundSrc) : _backgroundSrc, {allowMalformedURL:true, onComplete:backgroundLoaded});
				img.load();
			}							
		}
		...
		private function backgroundLoaded(e:LoaderEvent):void {
			...
			onStatus(MediaStatus.LOADED, imageFileLoaded);
		}
		...
		private function setDisplay():void {
			...
			if (backgroundImage && backgroundImage.parent != this) {				

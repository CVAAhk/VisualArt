package com.gestureworks.cml.managers 
{
	...
	{		
		 ...
		 */
		protected function reorder(event:StateEvent):void {
			var res:TouchContainer = event.target as TouchContainer;
			type = definition(res);
			
			//adjust priority based on visibility
			if (event.property == "visible") {
				if (event.value) {   //assign lowest priority to most recently visible
					tail(_resources[type], res);
				}
				else{				//assign highest priority to most recently invisible

    ...
    */
    ...
    function processResult(result){
        ...
        for(var i=0; i<count; i++){
            res = result[i] || result;
            if(res.item){  //file
                requestComplete({item: res.item.id, context: result.context, media: res.file_urls.original, thumb: res.file_urls.original, media_type: mediaType(res.file_urls.original)});
            }
